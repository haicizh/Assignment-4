...
public class Main {
			...
			case ValueIteration:
				runAlgorithm(analysis, problem, domain, hashingFactory, initialState, new PlannerFactory() {
					...
					public Planner createPlanner(int episodeIndex, SADomain domain, HashableStateFactory hashingFactory, SimulatedEnvironment simulatedEnvironment) {
						return new ValueIteration(domain, 0.8, hashingFactory, 0.0001, episodeIndex);
					}
				}, algorithm);
				break;
			case PolicyIteration:
				runAlgorithm(analysis, problem, domain, hashingFactory, initialState, new PlannerFactory() {
					...
					public Planner createPlanner(int episodeIndex, SADomain domain, HashableStateFactory hashingFactory, SimulatedEnvironment simulatedEnvironment) {
						 ...
						 * iterations).
						 */
						return new PolicyIteration(domain, 0.8, hashingFactory, 0.0001, problem.getNumberOfIterations(Algorithm.ValueIteration), episodeIndex);
					}
				}, algorithm);
				break;
			default:
				runAlgorithm(analysis, problem, domain, hashingFactory, initialState, new PlannerFactory() {
					...
					@Override
					public Planner createPlanner(int episodeIndex, SADomain domain, HashableStateFactory hashingFactory, SimulatedEnvironment simulatedEnvironment) {
						QLearning agent = new QLearning(domain, 0.8, hashingFactory, 0.3, 0.1);
	...
	private static Problem createProblem1() {
		...
		numIterationsHashMap.put(Algorithm.ValueIteration, 300);
		numIterationsHashMap.put(Algorithm.PolicyIteration, 200);
		 ...
		 * is happy).
		 */
		return new Problem(map, numIterationsHashMap, -1,0, hazardRewardsHashMap);
	}
	...
	private static Problem createProblem2() {
		...
		HashMap<Algorithm, Integer> numIterationsHashMap = new HashMap<Algorithm, Integer>();
		numIterationsHashMap.put(Algorithm.ValueIteration, 30);
		numIterationsHashMap.put(Algorithm.PolicyIteration, 15);
		numIterationsHashMap.put(Algorithm.QLearning, 5000);
